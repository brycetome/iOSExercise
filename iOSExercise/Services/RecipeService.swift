...
public func fetchAllRecipes() async throws -> [RecipeDTO] {
    return try await fetchRecipesFromURLString(urlString: AllRecipes)
}

public func fetchMalformedRecipes() async throws -> [RecipeDTO] {
    return try await fetchRecipesFromURLString(urlString: MalformedRecipes)
}

public func fetchEmptyRecipes() async throws -> [RecipeDTO] {
    return try await fetchRecipesFromURLString(urlString: EmptyData)
}

private func fetchRecipesFromURLString(urlString: String) async throws -> [RecipeDTO] {
